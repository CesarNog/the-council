...
### Rate limiting
- [ ] `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN` — **not just a nice-to-have.** Without these, rate limiting falls back to the Cloudflare KV path, which is read-then-write with no atomic compare-and-swap. A synchronous in-memory guard now blocks concurrent bursts that land on the same warm function instance (proven in `api/_rateLimit.test.js`'s concurrency tests, including the KV-fully-down case), but bursts spread across *different* instances can still each be admitted up to the limit per instance. Upstash's sliding-window limiter is atomic across all instances and is what actually protects the shared Groq TPM budget under real traffic.
